...
class ROIObject:
    """A single region-of-interest polygon and its assigned ID.

    Stores the polygon vertices as an integer numpy array together with an
    optional region ID and centroid. A newly drawn ROI starts unassigned
    (``roi_id=0``, ``assigned=False``) until the user explicitly assigns it
    an ID through the editor UI.

    Args:
        pts: Sequence of (x, y) vertex coordinates for the polygon.
        roi_id: Initial region ID. Defaults to 0 (unassigned).
    """
    ...
    def move(self, dx, dy):
        ...
        Args:
            dx: Shift along the x-axis (cast to int).
            dy: Shift along the y-axis (cast to int).
        """
        self.pts   += [int(dx), int(dy)]
        self.center = np.mean(self.pts, axis=0)

    def rotate(self, angle_deg):
        """Rotate the polygon in place about its centroid.

        Args:
            angle_deg: Rotation angle in degrees, applied counter-clockwise
                via a standard 2D rotation matrix.
        """
    ...
    def scale(self, factor):
        """Scale the polygon in place about its centroid.

        Args:
            factor: Scale factor applied to each vertex's distance from the
                centroid; values greater than 1 grow the polygon, values
                less than 1 shrink it.
        """
...
class ImageCanvas(QWidget):
    """Interactive canvas that renders the base image, ROIs, and intensity overlay.

    Handles all mouse and keyboard interaction for drawing, selecting,
    moving, resizing, and deleting ROIs, plus mouse-wheel zoom and
    middle-button panning. Emits ``roi_changed`` whenever the ROI list or
    selection changes so the parent window can refresh its status display.

    Args:
        rm: The owning `ROIMaker` instance whose image and ROI list are
    ...
    def wheelEvent(self, e):
        """Zoom the canvas in or out, keeping the point under the cursor fixed.

        Zoom factor changes by ±15% per wheel step and is clamped to the
        range [1.0, 20.0]. The pan offset is adjusted so the image-space
        point under the cursor stays under the cursor after zooming.

        Args:
            e: The Qt wheel event; ``angleDelta().y()`` determines zoom
                direction (positive zooms in, negative zooms out).
        """
    ...
    def reset_zoom(self):
        """Reset zoom to 1x and clear any panning offset, then repaint."""
        self._zoom  = 1.0
        self._pan_x = 0.0
        self._pan_y = 0.0
        self.update()
    ...
    def paintEvent(self, _):
        """Render the base image, intensity overlay, ROIs, and any drawing preview.

        Draws (in order) the background pixmap if `~ROIMaker.show_bg` is
        enabled, the intensity threshold overlay if active, every ROI in
        `~ROIMaker.rois` (with the selected one highlighted), and finally an
        in-progress drawing preview if the user is currently drawing.

        Args:
            _: The Qt paint event (unused).
        """
        self._recompute_transform()
        p = QPainter(self)
    ...
    def mousePressEvent(self, e):
        """Start panning, ID-assignment, resizing, moving, or drawing based on mode and hit-test.

        Middle-button press starts panning. For a left-button press: in
        'assign' mode, clicking an unassigned ROI gives it the next
        sequential ID; otherwise, a resize starts if a selection handle is
        hit, a move starts if an existing ROI is hit, and a new draw
        operation starts if the current tool is not 'select'.

        Args:
            e: The Qt mouse press event.
        """
    ...
    def mouseMoveEvent(self, e):
        """Update the active pan, resize, move, or freehand-draw operation and refresh the cursor.
    ...
    def mouseReleaseEvent(self, e):
        """Finish the active pan, resize, move, or draw operation.

        When a draw operation finishes with at least 3 finalized points, a
        new `ROIObject` is appended to `~ROIMaker.rois` as unassigned.

        Args:
            e: The Qt mouse release event.
        """
        if e.button() == Qt.MiddleButton:
            self._panning = False
    ...
    def keyPressEvent(self, e):
        """Delete the selected ROI on Delete/Backspace, or reset zoom on Home.

        Args:
            e: The Qt key press event.
        """
...
class ROIApp(QMainWindow):
    """Main editor window hosting the sidebar controls and the drawing canvas.

    Wires together the `ImageCanvas`, the tool/output-type/intensity-filter
    sidebar controls, and the status bar, and drives all edits back onto the
    owning `ROIMaker` instance.

    Args:
        rm: The `ROIMaker` instance this window edits.
    """
    ...
    def keyPressEvent(self, e):
        """Dispatch keyboard shortcuts for tools, background toggle, delete, save, cancel, and zoom reset.
...
class ROIMaker:
    """Programmatic entry point for the interactive ROI Maker editor.

    Wraps a 2D intensity image, launches the interactive PySide6 editor via
    `draw`, and generates/saves binary and multi-ID region masks from the
    ROIs drawn (or loaded). If a mask file already exists at `save_path`,
    it is loaded and converted back into ROI objects on construction.
    ...
    Args:
        image_2d: 2D array of intensity values used as the background image
            and, when the intensity filter is enabled, as the source for
            threshold-based masks.
        save_path: Path (``.npy``) where the resulting mask(s) will be
            saved. Defaults to ``"masks/mask.npy"``. If a file already
            exists at this path, it is loaded via `load_mask` during
            initialization.
    """
    ...
    def load_mask(self, path: str):
        """Load a previously saved mask file and rebuild ROI objects from it.

        Each unique non-zero value in the loaded array is treated as a
        region ID; its contours (area > 5 px) are extracted with OpenCV and
        converted into `ROIObject` instances already marked as assigned.
        `assign_counter` is updated to stay above the highest loaded ID.

        Args:
            path: Path to the ``.npy`` mask file to load.

        Raises:
            Nothing: any failure while loading or parsing the file (missing
                file, bad shape, etc.) is caught internally and logged to
                stdout via ``print``; ``self.rois`` is left as-is on error.
        """
        try:
            loaded = np.load(path)
    ...
    def save_masks(self):
        """Save the current ROIs (and optionally the intensity mask) to disk.

        The file(s) written depend on `mask_type`:

        - ``'binary'``: a single binary mask saved to `save_path`.
        - ``'multi'``: a single multi-ID mask saved to `save_path`.
        - ``'both'``: a binary mask saved to ``<stem>_binary.npy`` and a
          multi-ID mask saved to ``<stem>_multi.npy``.

        If `intensity_active` is True, the intensity threshold mask is
        additionally saved to ``<stem>_intensity.npy`` regardless of
        `mask_type`. The destination directory is created if it does not
        already exist, and a confirmation message is printed for each file
        written.
        """
        stem, _ = os.path.splitext(os.path.abspath(self.save_path))
        os.makedirs(os.path.dirname(stem) or ".", exist_ok=True)
