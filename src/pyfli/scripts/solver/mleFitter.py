...
class MLEFLIFitter(BaseFLIFitter):
    ...
    def _post_process(self, popt, jac, status, model_type, pcov=None, manual_stat=None, manual_perr=None):
        ...
        ssr, chi_sq_report, _, r_sq = compute_fli_stats(final_model, data, len(popt))

        dof = max(len(data) - len(popt), 1)
        # For MLE/Poisson, stat_val is the deviance returned by the optimizer;
        # for Pearson/Neyman/LS, fall back to Pearson chi_sq for consistency.
        stat_val = manual_stat if manual_stat is not None else chi_sq_report
        red_stat = stat_val / dof
